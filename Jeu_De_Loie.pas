...
BEGIN
	...
	REPEAT//Grande boucle repeat, elle se répete jusqu'a se que joueur = 66(fin).
		...
		begin
			...
			REPEAT//Petite boucle repeat elle se répete si le joueurs rentre un nombre plus petit que 2 ou plus grand que 12

				begin	

					writeln ('Vous etes sur la case ',' [',joueur,'] ', 'Appuyer sur entrer pour continuer');
					readln ();
					writeln ('lancez les deux des (Choisiez un nombre entre 2 et 12)');
					readln (de);
					if ((de > maxD) and (de < minD)) then
						writeln (' Entrez un nombre valide !');
						
				end
				until ((de <= maxD) and (de >= minD));
